...
type Face =
  ...
  , windowSecs :: Number
  -- | Whether a loop's layers are alternates — one sounds at a time, the
  -- | module's own way: the Layer knob. The rule is the daemon's (`alt`);
  -- | the page's part is to set it on a loop before recording into it, and
  -- | clicking a layer then makes it the one that sounds. Off, every layer
  -- | sounds and a checkbox parks one — the pedalboard's way.
  , alternates :: Boolean
...
-- | Whether the daemon, as it reports itself, suits this face. Empty is
-- | "yes"; each sentence is one thing to change and how. All of them, not
-- | the first: a daemon started for the pedalboard is usually wrong twice.
shapeNotes :: Face -> LooperState -> Array String
shapeNotes f top = Array.catMaybes
  [ if top.maxLayers < f.layers
